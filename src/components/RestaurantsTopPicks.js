...
const RestaurantsTopPicks = ({topPicks}) => {
  return (
    <div className='flex pb-12 overflow-auto '>
      {topPicks && <div className='flex gap-4'>
        {
          topPicks?.map((pick)=>{
            const {dish, creativeId} = pick;
            const {info} = dish;
            const {finalPrice, price} = info;
            return <div className='relative flex w-64 h-64' >
                  ...
                  <div >
                    <span className='absolute left-3 bottom-5 text-md text-white'>₹ {price/100}</span>

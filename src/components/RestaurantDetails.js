...
const RestaurantDetails = ({details}) => {
  const {name, avgRatingString, totalRatingsString, costForTwoMessage, cuisines, areaName, sla, cloudinaryImageId, city, id, latLong, slugString,labels} = details?.info
  ...
  return (
     <div className='relative mt-4 '>
      ...
      <span className='flex gap-2 '>
        ...
        <div className='text-orange-600 font-bold text-sm pb-2'>{cuisines.join(', ')}</div>
        <li className='font-bold text-sm'>Outlet <span className='text-gray-500 text-sm'>{' '+areaName}</span></li>

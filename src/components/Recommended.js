...
const Recommended = ({recommend}) => {
  ...
  return (
    <div className='pt-4 pb-8'>
      <div className='font-bold text-lg'>{title+' '}{'('+itemCards?.length+')'}</div>
      <div className='divide-y divide-slate-300'>{
        itemCard?.length > 0 && itemCard?.map((rec)=>{
          const {name, description, price, ratings,isVeg,isBestseller, addons} = rec?.card?.info
          return <div className='py-5 flex'>
            <div className='w-9/12'>
                <div className='flex items-center gap-1'>
                  <span>{ isVeg === 1 ? <img className='w-5' src={veg}/> : <img className='w-5' src={nonVeg}/>}</span>
                  <span>{isBestseller && <img className='h-4' src={bestseller}/>}</span>
                </div>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  <span className='text-sm font-semibold'>{ ratings?.aggregatedRating?.rating + ' ('+ ratings?.aggregatedRating?.ratingCountV2+')' }</span>
                </div>
                 }
            ...
            <div className='w-3/12'>
              <div className='flex justify-end relative' >
                  <img style={{backgroundColor: 'rgb(229, 241, 211)'}} className='rounded-xl ' src={'https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_156,h_144/'+rec?.card?.info?.imageId} alt='Food Image'/>
                  <div className='absolute -bottom-3 bg-white hover:bg-gray-200 py-2 px-10 text-green-600 right-4 border rounded-lg shadow-md text-lg font-bold'>
                    <button onClick={addons ? (e)=> addAddOns(rec?.card?.info) :(e)=>addCart(rec?.card?.info) }>
                      ADD
                    </button>
                  </div>
                  </div>
                  {addons && <div className='text-xs text-center font-semibold pl-10 ml-2 pt-4 text-gray-400'>Customisable</div>}
            </div>
          </div>
        })
        }
